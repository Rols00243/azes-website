    ...
    <div className="bg-[#040810] min-h-screen">
      ...
      <section className="bg-[#0a1628] border-b border-white/[0.06] sticky top-20 z-30">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex flex-wrap gap-4 items-center">
          <div className="flex flex-col gap-1">
            <label className="text-xs font-semibold text-white/40 uppercase tracking-wider">Zone</label>
            <select
              value={selectedZone}
          ...
          <div className="flex flex-col gap-1">
            <label className="text-xs font-semibold text-white/40 uppercase tracking-wider">Secteur</label>
            <select
              value={selectedSecteur}
              onChange={(e) => setSelectedSecteur(e.target.value)}
              className="text-sm bg-white/[0.06] border border-white/10 text-white rounded-lg px-3 py-2 focus:outline-none focus:border-white/30"
            >
              {secteurs.map((s) => <option key={s} className="bg-[#0a1628]">{s}</option>)}
            </select>
          </div>
          <div className="flex flex-col gap-1">
            <label className="text-xs font-semibold text-white/40 uppercase tracking-wider">État</label>
            <select
              value={selectedEtat}
              onChange={(e) => setSelectedEtat(e.target.value)}
              className="text-sm bg-white/[0.06] border border-white/10 text-white rounded-lg px-3 py-2 focus:outline-none focus:border-white/30"
            >
              {etats.map((e) => <option key={e} className="bg-[#0a1628]">{e}</option>)}
            </select>
          </div>
          <div className="ml-auto text-sm text-white/40 self-end pb-2">{filtered.length} projet{filtered.length !== 1 ? 's' : ''}</div>
        </div>
      </section>
      ...
      <section className="py-12 bg-[#040810]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {filtered.length === 0 ? (
            <div className="text-center py-20 text-white/40">Aucun projet correspondant à vos filtres.</div>
          ) : (
                  ...
                  <div className="p-7">
                    <div className="flex flex-col lg:flex-row gap-8">
                      <div className="flex-1">
                        ...
                        <h2 className="text-xl font-bold text-white mb-2">{projet.titre}</h2>
                        ...
                        <p className="text-white/50 text-sm leading-relaxed mb-5 line-clamp-2">{projet.description}</p>
                        <div className="mb-5">
                      ...
                      <div className="lg:w-52 flex flex-col gap-3">
                        <div className="bg-white/[0.05] border border-white/[0.08] rounded-xl p-4 text-center">
                          <div className="text-2xl font-bold text-white">{projet.avancement}%</div>
                          <div className="text-xs text-white/40 mt-0.5">Avancement</div>
                        </div>
                        <div className="bg-white/[0.05] border border-white/[0.08] rounded-xl p-4 text-center">
                          <div className="font-bold text-[#2A7A4B]">{projet.investissement}</div>
                          <div className="text-xs text-white/40 mt-0.5">Investissement</div>
                        </div>
                        <Link
                        ...
                        <div className="text-[10px] text-center text-white/25">
