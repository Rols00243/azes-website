...
const inp = "w-full bg-white/[0.06] border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder:text-white/30 focus:outline-none focus:border-white/30 focus:bg-white/[0.09] transition-all [&>option]:bg-[#0a1628]"
...
function SuccessMsg() {
  return (
    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
      ...
      <h3 className="font-bold text-white text-xl mb-2">Envoyé avec succès !</h3>
      <p className="text-white/50 text-sm">Notre équipe vous répondra dans les 48 heures ouvrées.</p>
    </motion.div>
  )
}
      ...
      <AnimatePresence>
        {result === 'found' && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            className="bg-[#2A7A4B]/15 border border-[#2A7A4B]/30 rounded-xl p-5">
            <div className="font-bold text-[#2A7A4B] mb-2">Dossier trouvé ✓</div>
            <div className="text-sm text-white/60 space-y-1">
              <div>Référence : <strong className="text-white">{ref}</strong></div>
              <div>Statut : <strong className="text-white">En cours d'instruction</strong></div>
        ...
        {result === 'notfound' && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            className="bg-red-500/10 border border-red-500/20 rounded-xl p-5 text-sm text-red-400">
  ...
  return (
    <div>
        ...
        {steps.map((s, i) => (
          <div key={s} className="flex-1 text-center">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center mx-auto mb-1.5 text-sm font-bold transition-all ${
              i < step ? 'bg-[#2A7A4B] text-white' : i === step ? 'bg-[#C4894A] text-white' : 'bg-white/[0.08] text-white/30'
            }`}>
              {i < step ? <CheckCircleIcon className="w-5 h-5" /> : i + 1}
            </div>
            <div className={`text-xs font-medium transition-colors ${i <= step ? 'text-white/70' : 'text-white/25'}`}>{s}</div>
          </div>
        ))}
      </div>
      ...
      <div className="mb-6">
        ...
        {step === 1 && (
          <div className="space-y-4">
            <p className="text-sm text-white/50 mb-4">Veuillez joindre tous les documents requis pour l'instruction de votre dossier.</p>
            ...
            <div className="bg-[#1B4F8C]/15 border border-[#1B4F8C]/25 rounded-xl p-4 text-sm text-white/60">
              <strong className="text-white/80">Documents requis :</strong>
        ...
        {step === 2 && (
          <div className="bg-white/[0.04] border border-white/[0.08] rounded-xl p-6 text-sm text-white/60 space-y-3">
            <h3 className="font-semibold text-white mb-4">Résumé de votre dossier</h3>
            <div className="grid grid-cols-2 gap-3">
              <div><span className="text-white/40">Fichiers joints :</span> <strong className="text-white">{files.length} fichier(s)</strong></div>
              <div><span className="text-white/40">Statut :</span> <strong className="text-[#2A7A4B]">Prêt à soumettre</strong></div>
            </div>
            <p className="text-white/35 text-xs pt-2">Vérifiez vos informations avant de soumettre définitivement votre dossier.</p>
          </div>
        )}
        {step === 3 && (
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
            className="bg-[#2A7A4B]/15 border border-[#2A7A4B]/30 rounded-xl p-8 text-center">
            <CheckCircleIcon className="w-14 h-14 text-[#2A7A4B] mx-auto mb-4" />
            <h3 className="font-bold text-white text-lg mb-2">Dossier soumis avec succès !</h3>
            <p className="text-white/60 text-sm mb-3">Référence : <strong className="text-white">AZES-2026-{Math.floor(Math.random() * 90000) + 10000}</strong></p>
            <p className="text-white/40 text-xs">Notre équipe reviendra vers vous dans les 5 jours ouvrés.</p>
          </motion.div>
        )}
      </div>
      ...
      <div className="flex justify-between">
        <button onClick={() => setStep(s => Math.max(0, s - 1))} disabled={step === 0 || step === 3}
          className="flex items-center gap-2 px-5 py-2.5 border border-white/10 rounded-xl text-sm font-medium disabled:opacity-30 hover:bg-white/[0.05] text-white/60 transition-colors">
      ...
      <section className="relative py-20 overflow-hidden">
        ...
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-0 left-0 w-[500px] h-[400px]"
            style={{ background: 'radial-gradient(ellipse at left top, rgba(27,79,140,0.12) 0%, transparent 65%)' }} />
          <div className="absolute bottom-0 right-0 w-[400px] h-[300px]"
            style={{ background: 'radial-gradient(ellipse at right bottom, rgba(196,137,74,0.10) 0%, transparent 65%)' }} />
        </div>
          ...
          <AnimatePresence mode="wait">
            {!activeId ? (
              <motion.div key="grid" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                ...
                {demarches.map((d, i) => (
                  <motion.button
                    ...
                    className="group text-left relative bg-white/[0.04] border border-white/[0.08] rounded-2xl p-6 hover:border-white/20 hover:bg-white/[0.07] hover:-translate-y-1 transition-all duration-300 overflow-hidden"
                  >
                    ...
                    <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity"
                      style={{ background: `radial-gradient(circle at 30% 30%, ${d.color}12 0%, transparent 60%)` }} />
                    <div className="relative">
                      <div className="w-12 h-12 rounded-2xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform"
                        style={{ backgroundColor: `${d.color}20`, border: `1px solid ${d.color}30` }}>
                        <d.icon className="w-6 h-6" style={{ color: d.color }} />
                      </div>
                      <h3 className="font-bold text-white/80 mb-2 group-hover:text-white transition-colors text-sm">{d.title}</h3>
              ...
              <motion.div key="detail" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                ...
                <div className="relative bg-white/[0.04] border border-white/[0.08] rounded-2xl p-8 overflow-hidden">
                  ...
                  <div className="flex items-center gap-4 mb-8 pb-6 border-b border-white/[0.07]">
                    {activeDemarche && (
                      <div className="w-12 h-12 rounded-xl flex items-center justify-center"
                        style={{ backgroundColor: `${activeDemarche.color}20`, border: `1px solid ${activeDemarche.color}30` }}>
                        <activeDemarche.icon className="w-6 h-6" style={{ color: activeDemarche.color }} />
                      </div>
                    )}
                    <div>
                      <h2 className="text-xl font-bold text-white">{activeDemarche?.title}</h2>
                      <p className="text-sm text-white/45">{activeDemarche?.desc}</p>
                    </div>
                  </div>
                  ...
                  {activeId === 'guichet' && (
                    <div className="space-y-4 text-sm">
                      <div className="bg-white/[0.04] border border-white/[0.07] rounded-xl p-5 space-y-3 text-white/60">
                        <div><span className="font-semibold text-white/80">Téléphone :</span> <a href="tel:+243812345678" className="text-[#C4894A] hover:underline">+243 81 234 5678</a></div>
                        <div><span className="font-semibold text-white/80">Email :</span> <a href="mailto:guichet@azes.cd" className="text-[#C4894A] hover:underline">guichet@azes.cd</a></div>
                        <div><span className="font-semibold text-white/80">Horaires :</span> Lun–Ven 08h00–17h00</div>
                        <div><span className="font-semibold text-white/80">Adresse :</span> Boulevard du 30 Juin, Gombe, Kinshasa</div>
                      </div>
