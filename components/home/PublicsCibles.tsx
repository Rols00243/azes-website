...
export default function PublicsCibles() {
  return (
    /* ── SECTION ÉCLAIRÉE ⑩ ── lumière mixte ambre + vert depuis le centre-haut */
    <section className="relative py-20 overflow-hidden">

      {/* Base illuminée */}
      <div className="absolute inset-0 bg-[#071d30]" />

      {/* Photo industrielle — foule / personnes en action */}
      <div
        className="absolute inset-0 opacity-[0.12]"
        style={{
          backgroundImage: `url('https://images.unsplash.com/photo-1522071820081-009f0129c71c?w=1400&q=60')`,
          backgroundSize: 'cover',
          backgroundPosition: 'center 25%',
        }}
      />

      {/* ── Lumières directionnelles mixtes depuis le haut */}
      <div className="absolute inset-0 pointer-events-none">
        {/* Rayon ambre — haut gauche */}
        <div className="absolute"
          style={{
            inset: 0,
            background: 'radial-gradient(ellipse 65% 80% at 10% -10%, rgba(196,137,74,0.24) 0%, rgba(196,137,74,0.08) 45%, transparent 65%)',
          }} />
        {/* Rayon vert — haut droite */}
        <div className="absolute"
          style={{
            inset: 0,
            background: 'radial-gradient(ellipse 60% 70% at 92% -5%, rgba(42,122,75,0.22) 0%, rgba(42,122,75,0.07) 45%, transparent 65%)',
          }} />
        {/* Halo central doux */}
        <div className="absolute"
          style={{
            inset: 0,
            background: 'radial-gradient(ellipse 60% 40% at 50% 0%, rgba(255,255,255,0.04) 0%, transparent 60%)',
          }} />
      </div>

      {/* Lignes de bord */}
      <div className="absolute top-0 inset-x-0 h-px"
        style={{ background: 'linear-gradient(90deg, rgba(196,137,74,0.25), rgba(255,255,255,0.10) 50%, rgba(42,122,75,0.20))' }} />
        ...
        <div className="grid grid-cols-3 sm:grid-cols-3 lg:grid-cols-9 gap-3">
          {publics.map((p, i) => (
            ...
            >
              <Link
                href={p.href}
                className="flex flex-col items-center gap-3 p-4 rounded-2xl bg-white/[0.08] border border-white/[0.13] hover:border-white/25 hover:bg-white/[0.13] transition-all group text-center"
              >
                <div className="w-12 h-12 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform"
                  style={{ backgroundColor: `${p.color}20` }}>
                  <p.icon className="w-6 h-6" style={{ color: p.color }} />
                </div>
                <span className="text-xs font-semibold text-white/60 group-hover:text-white/90 leading-tight transition-colors">{p.label}</span>
              </Link>
            </motion.div>
          ))}
        </div>
