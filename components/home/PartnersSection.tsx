...
export default function PartnersSection() {
  return (
    <section className="relative py-24 overflow-hidden" aria-label="Partenaires">
      {/* Night bg */}
      ...
      <div
        ...
        style={{
          backgroundImage: `url('https://images.unsplash.com/photo-1518709766631-a6a7f45921c3?w=1400&q=50')`,
          backgroundSize: 'cover',
          backgroundPosition: 'center',
        }}
      />

      {/* Amber glow — center top */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[300px] pointer-events-none"
        style={{ background: 'radial-gradient(ellipse, rgba(196,137,74,0.15) 0%, transparent 70%)' }} />
      {/* Green glow — bottom */}
      <div className="absolute bottom-0 right-0 w-[600px] h-[300px] pointer-events-none"
        style={{ background: 'radial-gradient(ellipse at right bottom, rgba(42,122,75,0.12) 0%, transparent 70%)' }} />
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3 mb-10">
          {partners.map((p, i) => (
            <motion.div
              ...
              className="group relative bg-white/[0.03] border border-white/[0.07] rounded-2xl p-4 hover:border-white/20 hover:bg-white/[0.06] transition-all cursor-default text-center overflow-hidden"
            >
              {/* Corner accent */}
              ...
              <div className="font-bold text-white/80 text-xs mb-1 leading-tight">{p.name}</div>
              ...
              <div className="mt-2.5 inline-block px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider"
                style={{ color: categoryColors[p.category] ?? p.color, backgroundColor: `${categoryColors[p.category] ?? p.color}15`, border: `1px solid ${categoryColors[p.category] ?? p.color}30` }}>
                {p.category}
              </div>
            </motion.div>
          ))}
